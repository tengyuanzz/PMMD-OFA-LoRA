...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None,
        ...
        self.enc_in = self.data_x.shape[-1]
        print('enc_in',self.data_x.shape, self.enc_in)
        self.tot_len = len(self.data_x) - self.seq_len - self.pred_len + 1

    def __read_data__(self):
        self.scaler = StandardScaler()
        df_raw = pd.read_csv(os.path.join(self.root_path, self.data_path)).head(1000)
        cols_to_keep = list(df_raw.columns[:50]) + [df_raw.columns[-1]]
...
class Dataset_Custom2(Dataset):
    ...
    def __read_data__(self):
        ...
        cols = list(df_raw.columns)
        self.target_col_index = cols.index(self.target)
        print(self.target_col_index)
        cols.remove(self.target)
        cols.remove('date')
